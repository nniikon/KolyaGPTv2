#ifndef KGPT_ADDITION_H
#define KGPT_ADDITION_H
...
template <typename T>
std::unique_ptr<Operation<BasicMatrixData<T>>> Add(GenericMatrix<BasicMatrixData<T>>& dad,
                                                   GenericMatrix<BasicMatrixData<T>>& mom) {
    return std::make_unique<AdditionOperation<T>>(dad, mom);
}

template <typename T>
class AdditionOperation final : public BinaryOperation<BasicMatrixData<T>> {

using BinaryOperation<BasicMatrixData<T>>::dad_;
using BinaryOperation<BasicMatrixData<T>>::mom_;

public:
    AdditionOperation(GenericMatrix<BasicMatrixData<T>>& dad,
                      GenericMatrix<BasicMatrixData<T>>& mom)
